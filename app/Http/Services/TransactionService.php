...
class TransactionService
{
    ...
    return datatables()->eloquent($transaction)
            ...
            ->addColumn('memo', function ($data) {
                return '<a onclick="getEditModal(\'' . route('transaction.download', $data->id) . '\'' . ', \'#invoiceModal\')" href="#" class="d-block text-center min-w-130 text-decoration-underline fw-600 text-1b1c17" data-bs-toggle="modal" data-bs-target="#alumniPhoneNo">
                Download Receipt </a>';
            })
            ->addColumn('amount', function ($data) {
                return showPrice($data->amount);
    ...
    public function pendingTransactionList()
    {
        ...
        $data = Payment::join('gateways', 'gateways.id', '=', 'payments.gateway_id')
            ...
            ->whereNotIn('paymentable_type', ['App\Models\Campaign', 'App\Models\CommitteeNominationForm'])
            ->with('paymentable')
            ->with('gateway')
            ->with('bank')
            ->with('user')
