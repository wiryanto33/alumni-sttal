...
class RegisterController extends Controller
{
    ...
    */
    ...
    {
        $regForm = RegisterForm::where('tenant_id', getTenantId())->first();
        $rules = [
            ...
            "passing_year_id" => 'nullable',
            "id_number" => 'bail|nullable',
        ...
        if ($regForm->enable_passing_year) {
            $rules["passing_year_id"] = 'required';
        }
        if ($regForm->enable_role_number) {
            $rules["id_number"] = 'bail|required|min:1';
        }
        if ($regForm->enable_attachment) {
