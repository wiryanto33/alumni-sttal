...
<div class="zSidebar" data-background="{{ asset('assets/images/sidebar-bg.png') }}">
    ...
    <div class="zSidebar-fixed">
        <ul class="zSidebar-menu" id="sidebarMenu">
            ...
            @if((!isCentralDomain() || !isAddonInstalled('ALUSAAS')) && in_array(auth()->user()->role, [USER_ROLE_ADMIN, USER_ROLE_ALUMNI]))
                @if(auth()->user()->role == USER_ROLE_ALUMNI || auth()->user()->is_alumni == STATUS_ACTIVE)
                    <li>
                    ...
                    <li>
                        ...
                        <div class="collapse {{ $showEvent ?? '' }}" id="myEvent" data-bs-parent="#sidebarMenu">
                            <ul class="zSidebar-submenu">
                                @if (auth()->user()->role == USER_ROLE_ADMIN)
                                    @if(auth()->user()->is_alumni == STATUS_ACTIVE)
                    ...
                    @if(isAddonInstalled('ALUCOMMITTEE'))
                        <li>
                            <a href="#manage-alu-election" data-bs-toggle="collapse" role="button"
                               aria-expanded="{{ isset($showManageAluVote) ? 'true' : 'false' }}"
                               aria-controls="manage-alu-election"
                               class="d-flex align-items-center cg-10 {{ isset($showManageAluVote) ? 'active' : 'collapsed' }}">
                                <div class="d-flex">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25"
                                         fill="none">
                                        <path
                                            d="M2.60449 12.5C2.60449 8.09405 2.60449 5.89106 3.85238 4.42998C4.02958 4.2225 4.2228 4.02927 4.43028 3.85207C5.89137 2.60419 8.09436 2.60419 12.5003 2.60419C16.9063 2.60419 19.1093 2.60419 20.5703 3.85207C20.7778 4.02927 20.9711 4.2225 21.1482 4.42998C22.3962 5.89106 22.3962 8.09405 22.3962 12.5C22.3962 16.906 22.3962 19.109 21.1482 20.57C20.9711 20.7775 20.7778 20.9707 20.5703 21.1479C19.1093 22.3959 16.9063 22.3959 12.5003 22.3959C8.09436 22.3959 5.89137 22.3959 4.43028 21.1479C4.2228 20.9707 4.02958 20.7775 3.85238 20.57C2.60449 19.109 2.60449 16.906 2.60449 12.5Z"
                                            stroke="white" stroke-opacity="0.7" stroke-width="1.5"/>
                                        <path
                                            d="M15.3233 10.4803C15.2201 9.68585 14.3079 8.40229 12.6677 8.40226C10.7618 8.40223 9.95991 9.45775 9.79719 9.98551C9.54334 10.6914 9.59411 12.1428 11.828 12.301C14.6204 12.4989 15.739 12.8285 15.5967 14.5375C15.4543 16.2463 13.8978 16.6156 12.6677 16.5759C11.4375 16.5364 9.42478 15.9713 9.34668 14.4513M12.4725 7.28961V8.40604M12.4725 16.5657V17.7062"
                                            stroke="white" stroke-opacity="0.7" stroke-width="1.5"
                                            stroke-linecap="round"/>
                                    </svg>
                                </div>
                                <span class="">{{ __('Election') }} @if(env('APP_DEMO', false))
                                        <span class="fs-14 text-danger">(Addon)</span>
                                    @endif</span>
                            </a>

                            <div class="collapse {{ $showManageAluVote ?? '' }}" id="manage-alu-election"
                                 data-bs-parent="#sidebarMenu">
                                <ul class="zSidebar-submenu">
                                    <li>
                                        <a class="{{ $activeNominationApplyList ?? '' }}"
                                           href="{{route('committee.alumni.nominations.apply_list')}}">{{ __('Nomination') }}</a>
                                    </li>
                                    <li>
                                        <a class="{{ $activeAluVote ?? '' }}"
                                           href="{{route('committee.alumni.votes.list')}}">{{ __('Vote') }}</a>
                                    </li>
                                </ul>
                            </div>
                        </li>
                ...
                @endif
            @endif
            @if(in_array(auth()->user()->role, [USER_ROLE_ADMIN, USER_ROLE_ALUMNI]))
                @if(auth()->user()->role == USER_ROLE_ALUMNI || auth()->user()->is_alumni == STATUS_ACTIVE)
                    <li>
            ...
            @endif
            @if (auth()->user()->role == USER_ROLE_ADMIN || auth()->user()->role == USER_ROLE_BOARD_MEMBER)
                @if(!isCentralDomain() || !isAddonInstalled('ALUSAAS'))
                    @if (auth()->user()->role == USER_ROLE_ADMIN)
                        @canany(['Manage Alumni', 'Manage Membership', 'Manage Notice', 'Manage News', 'Manage Transaction', 'Manage Donation', 'Manage Committee', 'Manage Vote', 'Manage Moderator', 'Manage Website Settings', 'Manage Newsletter', 'Manage Application Setting', 'Manage Version Update'])
                        ...
                        @can('Manage Alumni')
                            <li>
                                <a href="#alumni-admin" data-bs-toggle="collapse" role="button"
                                   aria-expanded="{{ isset($showAdminAlumni) ? 'true' : '' }}"
                                   aria-controls="membership-admin"
                        ...
                        @can('Manage Membership')
                            <li>
                                <a href="#membership-admin" data-bs-toggle="collapse" role="button"
                                   aria-expanded="{{ isset($showMembership) ? 'true' : '' }}"
                                   aria-controls="membership-admin"
                                ...
                                <div class="collapse {{ $showMembership ?? '' }}" id="membership-admin"
                                    ...
                                    <ul class="zSidebar-submenu">
                                        ...
                                        <li><a class="{{ $activeMemberList ?? '' }}"
                                               href="{{ route('admin.membership.list') }}">{{ __('Member List') }}</a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                                ...
                                <div class="collapse {{ $showManageNotice ?? '' }}" id="manage-notice-menu"
                                    ...
                                    <ul class="zSidebar-submenu">
                                        <li><a class="{{ $activeNoticeCategory ?? '' }}"
                                               href="{{ route('admin.notices.categories.index') }}">{{ __('Category') }}</a>
                                        </li>
                                        <li><a class="{{ $activeManageNotice ?? '' }}"
                                               href="{{ route('admin.notices.index') }}">{{ __('Notice') }}</a></li>
                                    </ul>
                                </div>
                            </li>
                        @endcan
                                ...
                                <div class="collapse {{ $showManageNews ?? '' }}" id="manage-news-menu"
                                    ...
                                    <ul class="zSidebar-submenu">
                                        ...
                                        <li><a class="{{ $activeNewsCategory ?? '' }}"
                                               href="{{ route('admin.news.categories.index') }}">{{ __('Category') }}</a>
                                        </li>
                                ...
                                <div class="collapse {{ $showTransactionNotice ?? '' }}" id="manage-transaction-menu"
                                    ...
                                    <ul class="zSidebar-submenu">
                                        <li><a class="{{ $activePaymentNotice ?? '' }}"
                                               href="{{route('admin.transactions.pending.list')}}">{{ __('Pending Transaction') }}</a>
                                        </li>
                                        <li><a class="{{ $activeTransactionNotice ?? '' }}"
                                               href="{{route('admin.transactions.all.list')}}">{{ __('All Transaction') }}</a>
                                        </li>
                        ...
                        @if(isAddonInstalled('ALUDONATION'))
                            @can('Manage Donation')
                                <li>
                                    <a href="#manage-donation-menu" data-bs-toggle="collapse" role="button"
                                       aria-expanded="{{ isset($showManageDonationCampaign) ? 'true' : 'false' }}"
                                       aria-controls="manage-donation-menu"
                                       class="d-flex align-items-center cg-10 {{ isset($showManageDonationCampaign) ? 'active' : 'collapsed' }}">
                                        <div class="d-flex">
                                        ...
                                        <span class="">{{ __('Manage Donation') }} @if(env('APP_DEMO', false))
                                                <span class="fs-14 text-danger">(Addon)</span>
                                            @endif</span>
                                    </a>

                                    <div class="collapse {{ $showManageDonationCampaign ?? '' }}"
                                         id="manage-donation-menu"
                                         data-bs-parent="#sidebarMenu">
                                        <ul class="zSidebar-submenu">
                                            <li><a class="{{ $activeManageCategories ?? '' }}"
                                                   href="{{route('donation.admin.categories.index')}}">{{ __('Categories') }}</a>
                                            </li>
                                            <li><a class="{{ $activeManageCampaign ?? '' }}"
                                                   href="{{route('donation.admin.campaigns.index')}}">{{ __('Campaign') }}</a>
                                            </li>
                                            <li><a class="{{ $activeManageCampaignComment ?? '' }}"
                                                   href="{{route('donation.admin.campaigns.comments')}}">{{ __('Campaign Comment') }}</a>
                                            </li>
                                            <li><a class="{{ $activeManagePendingDonation ?? '' }}"
                                                   href="{{route('donation.admin.donations.pending')}}">{{ __('Pending Donation') }}</a>
                                            </li>
                                            <li><a class="{{ $activeManageDonationReport ?? '' }}"
                                                   href="{{route('donation.admin.donations.report')}}">{{ __('Donation Report') }}</a>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                    ...
                    @if(isAddonInstalled('ALUCOMMITTEE') )
                        @if(in_array(auth()->user()->role, [USER_ROLE_ADMIN, USER_ROLE_ALUMNI]))
                            @can('Manage Committee')
                                <li>
                                    <a href="#manage-committee-menu" data-bs-toggle="collapse" role="button"
                                       aria-expanded="{{ isset($showManageCommittee) ? 'true' : 'false' }}"
                                       aria-controls="manage-committee-menu"
                                       class="d-flex align-items-center cg-10 {{ isset($showManageCommittee) ? 'active' : 'collapsed' }}">
                                        <div class="d-flex">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25"
                                                 viewBox="0 0 25 25"
                                                 fill="none">
                                        ...
                                        <span class="">{{ __('Manage Committee') }} @if(env('APP_DEMO', false))
                                                <span class="fs-14 text-danger">(Addon)</span>
                                            @endif</span>
                                    </a>

                                    <div class="collapse {{ $showManageCommittee ?? '' }}" id="manage-committee-menu"
                                         data-bs-parent="#sidebarMenu">
                                        <ul class="zSidebar-submenu">
                                            <li>
                                                <a class="{{ $activeManageCategories ?? '' }}"
                                                   href="{{route('committee.admin.categories.index')}}">{{ __('Committee Category') }}</a>
                                            </li>
                                            <li>
                                                <a class="{{ $activeDesignationList ?? '' }}"
                                                   href="{{route('committee.admin.designation.index')}}">{{ __('Designation') }}</a>
                                            </li>
                                            <li>
                                                <a class="{{ $activeElectionList ?? '' }}"
                                                   href="{{route('committee.admin.election.index')}}">{{ __('Election') }}</a>
                                            </li>
                                            <li>
                                                <a class="{{ $activeCandidateFlagList ?? '' }}"
                                                   href="{{route('committee.admin.candidate-flag.index')}}">{{ __('Election Symbol') }}</a>
                                            </li>
                                            <li>
                                                <a class="{{ $activeNominationList ?? '' }}"
                                                   href="{{route('committee.admin.nominations.index')}}">{{ __('Nomination') }}</a>
                                            </li>
                                            <li>
                                                <a class="{{ $activeNominationTransaction ?? '' }}"
                                                   href="{{route('committee.admin.nominations.transaction')}}">{{ __('Nomination Transaction') }}</a>
                                            </li>
                                            <li>
                                                <a class="{{ $activeManageBoardMember ?? '' }}"
                                                   href="{{route('committee.admin.board_members.index')}}">{{ __('Board Member') }}</a>
                                            </li>
                                            <li>
                                                <a class="{{ $activeExecutiveCommittee ?? '' }}"
                                                   href="{{route('committee.admin.committee.index')}}">{{ __('Committee Member  ') }}</a>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                        ...
                        @can('Manage Vote')
                            <li>
                                <a href="#manage-vote-menu" data-bs-toggle="collapse" role="button"
                                   aria-expanded="{{ isset($showManageVote) ? 'true' : 'false' }}"
                                   aria-controls="manage-vote-menu"
                                   class="d-flex align-items-center cg-10 {{ isset($showManageVote) ? 'active' : 'collapsed' }}">
                                    <div class="d-flex">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25"
                                             viewBox="0 0 25 25"
                                             fill="none">
                                    ...
                                    <span class="">{{ __('Manage Vote') }} @if(env('APP_DEMO', false))
                                            <span class="fs-14 text-danger">(Addon)</span>
                                        @endif</span>
                                </a>

                                <div class="collapse {{ $showManageVote ?? '' }}" id="manage-vote-menu"
                                     data-bs-parent="#sidebarMenu">
                                    <ul class="zSidebar-submenu">
                                        <li>
                                            <a class="{{ $activePendingCandidate ?? '' }}"
                                               href="{{route('committee.admin.candidates.pending')}}">{{ __('Pending Candidate') }}</a>
                                        </li>
                                        <li>
                                            <a class="{{ $activeFinalCandidate ?? '' }}"
                                               href="{{route('committee.admin.candidates.final')}}">{{ __('Final Candidate') }}</a>
                                        </li>
                                        <li>
                                            <a class="{{ $activeManageCommitteeCandidateComment ?? '' }}"
                                               href="{{route('committee.admin.candidates-comments.index')}}">{{ __('Candidate Comments') }}</a>
                                        </li>
                                        <li>
                                            <a class="{{ $activeVoteList ?? '' }}"
                                               href="{{route('committee.admin.votes.index')}}">{{ __('Vote List') }}</a>
                                        </li>
                                        <li>
                                            <a class="{{ $activeElectionResultList ?? '' }}"
                                               href="{{route('committee.admin.election.result-list')}}">{{ __('Result List') }}</a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                    ...
                    @if (auth()->user()->role == USER_ROLE_ADMIN)
                        @can('Manage Moderator')
                            <li>
                                <a href="#manage-moderator-menu" data-bs-toggle="collapse" role="button"
                                   aria-expanded="{{ isset($showManageModerator) ? 'true' : 'false' }}"
                                   aria-controls="manage-moderator-menu"
                                   class="d-flex align-items-center cg-10 {{ isset($showManageModerator) ? 'active' : 'collapsed' }}">
                                    <div class="d-flex">
                                        <svg width="25" height="18" viewBox="0 0 25 18" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <path
                                    ...
                                    <span class="">{{ __('Manage Moderator') }}</span>
                                </a>

                                <div class="collapse {{ $showManageModerator ?? '' }}" id="manage-moderator-menu"
                                     data-bs-parent="#sidebarMenu">
                                    <ul class="zSidebar-submenu">
                                        <li>
                                            <a class="{{ $activeRole ?? '' }}"
                                               href="{{route('admin.roles.index')}}">{{ __('Role') }}</a>
                                        </li>
                                        <li>
                                            <a class="{{ $activeModerator ?? '' }}"
                                               href="{{route('admin.moderators.index')}}">{{ __('Moderator') }}</a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                        ...
                        @can('Manage Newsletter')
                            <li>
                                <a href="#subscriptionNewsLetter" data-bs-toggle="collapse" role="button"
                                   aria-controls="subscriptionNewsLetter"
                                   class="d-flex align-items-center cg-10 {{ isset($showNewsSubscriptionLetter) ? 'active' : 'collapsed' }}"
                                   aria-expanded="{{ isset($showNewsSubscriptionLetter) ? 'true' : 'false' }}">
                                    <div class="d-flex">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <rect x="3" y="6" width="18" height="15" rx="2"
                                                  stroke="white" stroke-width="1.5"/>
                                            <path d="M4 11H20" stroke="white" stroke-width="1.5"
                                                  stroke-linecap="round"/>
                                            <path d="M9 16H15" stroke="white" stroke-width="1.5"
                                                  stroke-linecap="round"/>
                                            <path d="M8 3L8 7" stroke="white" stroke-width="1.5"
                                                  stroke-linecap="round"/>
                                            <path d="M16 3L16 7" stroke="white" stroke-width="1.5"
                                                  stroke-linecap="round"/>
                                        </svg>
                                    </div>
                                    <span class="">{{ __('Subscription Newsletter') }}</span>
                                </a>
                                <div class="collapse {{ $showNewsSubscriptionLetter ?? '' }}"
                                     id="subscriptionNewsLetter"
                                     data-bs-parent="#sidebarMenu">
                                    <ul class="zSidebar-submenu">
                                        <li><a class="{{ $activeSubscriptionLetterEmail ?? '' }}"
                                               href="{{ route('admin.news-subscription-letter-email.list') }}">{{ __('Subscriber Email') }}</a>
                                        </li>
                                        <li><a class="{{ $activeEmailTemplateSubscription ?? '' }}"
                                               href="{{ route('admin.subscription-email-template.list') }}">{{ __('Email Template') }}</a>
                                        </li>
                                        <li><a class="{{ $activeEmailTemplateSubscriptionSendMail ?? '' }}"
                                               href="{{ route('admin.subscription-email-template.send-mail-list') }}">{{ __('Send Mail') }}</a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                            ...
                            <li>
                                ...
                                <div class="collapse {{ $showManageApplicationSetting ?? '' }}"
                                    ...
                                    <ul class="zSidebar-submenu">
                                        ...
                                        <li>
                                            <a class="{{ $activeRegFormSetting ?? '' }}"
                                               href="{{ route('admin.setting.registration-form-settings') }}">{{ __('Registration Form Setting') }}</a>
                                        </li>
